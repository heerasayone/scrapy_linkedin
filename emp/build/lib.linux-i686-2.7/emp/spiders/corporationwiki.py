...
class Employee_details(scrapy.Spider):
    ...
    def parse(self, response):
        f = resource_stream('emp', 'linked.csv')
        SIGNED_KEY = f.read()
    ...
    def parse_listing_page(self, response):
        ...
        if 'listing_count' not in meta:
          ...
          if listing_count:
            if int(listing_count[0]) <= 1000:
              listing_countt = listing_count[0]
        else:
          listing_countt = meta['listing_count']
        ...
        #pagination
        if listing_countt:
          if int(listing_countt) > 10:
            listing_count = int(listing_countt)-10
            meta['listing_count'] = listing_countt
            count = int(listing_countt)/10
